...
{	
	...
	while ((c = getopt (argc, argv, "vf:t:s:")) != -1)
	{
		...
		switch (c)
		{
			...
			case 't':
				...
				break;
			case 's':
        // we are using milliseconds here
  ...
  //remember seconds is milliseconds here, so changing to nsec
  //printf("1\n");
  //printf("%lld.%.9ld\n", (long long)endtime.tv_sec, endtime.tv_nsec);

  long ms = 1000000 * seconds;
  if ((endtime.tv_nsec + ms) > 999999999) {
    //normalize
    endtime.tv_sec += 1;
    endtime.tv_nsec = endtime.tv_nsec + ms - 1000000000;
  } else {
    endtime.tv_nsec += ms;
  }
